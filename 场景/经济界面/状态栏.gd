...
## 状态栏 — 经济界面。
## 数值与悬浮提示对齐原版 Economy.unity 顶部 плашка показателей：
##   Show_diplomacy_data_script.Repaint（data_old ±变化）+ OkoshkoScript.text_en（指标名）
...
func _refresh() -> void:
	...
	# 原版 red_t=1 挂在 num=6（国际声誉）元素上，但 Repaint 的变红分支要求 num != 6，
	# 因此原版该处永不转红；这里照抄为始终默认白色，不额外着色。
	_label("特工网络", "%.1f" % (float(eco.特工网络) / 10.0))
	_label("全球影响力", "%.1f" % eco.全球影响力)
	_label("预算", "%.1f" % (float(eco.预算) / 10.0))
	if w.empires.size() >= 2:
		# 原版 Repaint num=28/29：abs(rel/10).abs(rel%10)，恒一位小数；
		# display_meter 的 "-X.Y" 格式与其一致（旧实现整数位会丢十分位）。
		_label("与美国关系", w.display_meter(w.empires[0].relations))
		_label("与苏联关系", w.display_meter(w.empires[1].relations))
	_refresh_tooltips(w)
...
## 国际声誉（num=6）悬浮提示追加当前派系名。
## 原版 Show_diplomacy_data_script.Repaint num==6 分支读 data[6]（国际声望值）分档。
