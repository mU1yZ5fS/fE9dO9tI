...
func _mozambique_civil_active(w: WorldState) -> bool:
    ...
    if c == null:
        return false
    if c.parts.size() > 0 and c.parts[0]:
        return true
    return c.level_of_instability >= 500


## 缅甸内战：外交互动会将缅甸置为“内战中”，随后事件 663/664 才正式打开 82/83 战争。
func _burma_civil_active(w: WorldState) -> bool:
    if w.war_going(82) or w.war_going(83):
        return true
    var c := w.get_country_by_legacy_index(33)
    return c != null and c.内战中
