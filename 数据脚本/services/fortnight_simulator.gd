...
## 官方版 DLL 反编译（ts_official.cs:4537-4541）证实为直接写入：
## (data[34]-9307)/200 > 0 时 data[9] -= (data[34]-9307)/200。
## 旧转储 ptr 模式系反编译伪影，v0.3.3 误判为空操作，已恢复实扣。
...
func _fortnight_modifiers(
	...
	# 16/17 对苏/对美关系受损（官方版 DLL 反编译 mi_official.cs:903-940 逐字对齐）。
	# 扣除分支系数为 ×(对方power/500)（美方分支原版即错用苏联 power ÷1000，忠实保留）——
	# 除法在括号内，power<500 时因子整除为 0，早期禁运不扣特工。
	# 「我方禁运对方」（event7/8 结果4）走获利分支。v0.3.3 的乘除顺序与 mod17 取错
	# power 来源，已按官方 IL 修正；旧转储 ptr 模式系反编译伪影。
	...
	if gm._mod_active(w, GameConstants.Modifier.USA_EMBARGO):
		...
		var ussr_power_for_usa_embargo := w.empires[EmpireData.USSR].power if w.empires.size() > EmpireData.USSR else 0
		if usa_relation >= 500:
			w.modifiers[17].is_active = false
		elif int(w.completed_event_ids.get("event_007", 0)) != 4:
			# 原版此处即写 empires[1]（苏联）power ÷1000，系原作者复制粘贴笔误，忠实保留。
			@warning_ignore("integer_division")
			d.budget -= (500 - usa_relation) / 50 * (ussr_power_for_usa_embargo / 1000)
			@warning_ignore("integer_division")
			d.agents -= (500 - usa_relation) / 100 * (ussr_power_for_usa_embargo / 1000)
		else:
			@warning_ignore("integer_division")
	...
	if gm._mod_active(w, GameConstants.Modifier.OUR_MILITARY_ALLIANCE):
		...
		for c in w.countries:
			if c != null and c.has_tag("au") and (c.government == GameConstants.Government.SOCIALIST or c.sub_government == GameConstants.SubGovernment.LEFT_RADICAL) \
				...
				au59 += 1
		# 原版（改版 ModifiesInfuence.cs:2600-2712）四段计数与加成全部包在
		# `if allcountries[1].okb` 门控内：中国失去 okb（被逐出联盟等）时全部停发。
		if china59 != null and china59.has_tag("okb"):
			if okb59 > 0:
				d.army += okb59 * 3
				d.agents += okb59 * 2
...
func _fortnight_trade_balance(d: WorldState, w: WorldState) -> void:
	...
	if gm._mod_active(w, GameConstants.Modifier.BACKWARD_ECONOMY):
		d.income -= d.income / 6
	# 贸易平衡（官方版 DLL 反编译 ts_official.cs:4201-4212，直接写入）：
	# 顺差(23>24) budget+=(差)/10、people+=(差)/15；逆差 budget-=(差)/10、
	# people-=(差)/15、living-=(差)/20。旧转储 ptr 模式系反编译伪影，
	# v0.3.3 误判为空操作，已恢复。
	var diff := d.income - d.import_needs
	if d.income > d.import_needs:
		d.budget += diff / 10
	...
	# 伙伴数效应（ts_official.cs:4213-4222，直接写入）：
	# ≤10 → thought/agents -= (-9+伙伴数)；>18 → thought += 伙伴数-18。v0.3.3 误删，已恢复。
...
# ── 满意度/异见漂移（官方版 DLL 反编译 ts_official.cs:4224-4244） ──
# war_support>700：living-=(ws-500)/100、party+=(ws-500)/100、agents+=(ws-500)/100、
# 声誉<500 时+5；ws<400||ws>700 分支（实际仅<400 可达）：thought/party+=(500-ws)/100、
# agents-=(ws-500)/100、美苏关系各+=(500-ws)/100。全部直接写入；
# v0.3.3 误判 ptr 空操作只留了关系项，已恢复国家数值部分。
...
# ── 难度修正（官方版 DLL 反编译 ts_official.cs:6444-6529，双周块内直接写入）──
## 全部六档数值均为真实生效；旧转储 ptr 模式系反编译伪影，
## v0.3.3 误判 diff0-3 与 diff4 的 party/people 项为空操作，已逐档恢复。
