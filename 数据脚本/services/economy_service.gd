...
func calc_budget_planka(world: WorldState) -> int:
	if world == null:
		return 0
	var d := world
	var total: int = d.budget + d.reserve
	for i in range(W.I_BUDGET_ARMY, W.I_BUDGET_DIPLO + 1):
		total += d.get_data_by_index(i)
	if d.econ_system > 12:
		total -= (d.econ_system - 12) * (total / 10)
	return total
...
func adjust_budget(world: WorldState, category_idx: int, delta: int, notify: Callable = Callable()) -> bool:
	...
	if delta > 0:
		...
		# 允许加到 planka2/6+10；Shift/Ctrl 分支(:87/:93)才检查 data.get_data_by_index(idx)+50/100 <= planka2/6。
		if delta == 10:
			if d.get_data_by_index(category_idx) > limit6:
				return false
		elif d.get_data_by_index(category_idx) + delta > limit6:
			return false
	if delta < 0 and d.get_data_by_index(category_idx) < -delta:
		return false
	d.add_data_by_index(category_idx, delta)
	d.budget -= delta
	# 原版 Plusmisnus_script.payment：削减「高层福利」时按 4× 扣党内支持
	# -10 → 党支持-40；-50 → -200；-100 → -400
	if category_idx == W.I_BUDGET_ENVELOPE and delta < 0:
		d.party_support -= (-delta) * 4
	if notify.is_valid():
		notify.call()
	return true
...
func adjust_loan(world: WorldState, delta: int, notify: Callable = Callable()) -> bool:
	...
	if delta > 0:
		...
		if d.loan >= loan_limit:
			return false
		d.loan += delta
		d.budget += delta
		d.party_support -= delta
		# 借款思想自由：原版 data.thought_freedom += 10（Plusmisnus_script.cs:201），非 ×2.5
		d.thought_freedom += delta
		...
		if d.loan <= 0:
			return false
		var repay_amount := mini(-delta, d.loan)
		var mult := 1
		...
		d.loan -= repay_amount
		d.budget -= budget_cost
		# 还款党支持：原版 data.party_support += 5（Plusmisnus_script.cs:173/192），非 +10
		d.party_support += 5
