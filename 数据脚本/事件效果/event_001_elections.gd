...
##       data[125]/Debug.Log 跳过；惨败 data[35]=5→queue_ending_after_event(1)「人民的选择」(语义映射)。
...
func execute(context: Dictionary) -> void:
	...
	var arr := _compute_weights(ws, d)

	var head := ""
	var suffix := ""
	if opt == 0:
		_normalize_to_seats(ws, arr)
		_living_collapse_override(ws, d, true)   # 需 factions[0].is_enabled（:197）
		head = _resolve_branch(ws, d, true)
		...
		_normalize_to_seats(ws, arr)
		_pre_hold(ws)                                 # :284
		_living_collapse_override(ws, d, false)       # :288（无 is_enabled 守卫）
		head = _resolve_branch(ws, d, false)
		suffix = "，总席位3000"                        # :331
		...
		_normalize_to_seats(ws, arr)
		_pre_hold(ws)                                 # :387
		_living_collapse_override(ws, d, false)       # :391
		head = _resolve_branch(ws, d, false)
		suffix = "，总席位3000"                        # :434
	...
	context["result_text"] = head + "\n选举结果：\n" + _seats_report(ws, suffix)


## 五党得票权重（Event1.cs:40-155），整数除法逐字。
func _compute_weights(ws: WorldState, d: Array) -> Array:
	var f := ws.factions
...
func _normalize_to_seats(ws: WorldState, arr: Array) -> void:
	var total := 0
	for v in arr:
		total += int(v)
	if total <= 0:
...
func _living_collapse_override(ws: WorldState, d: Array, guard_enabled: bool) -> void:
	if d[W.I_LIVING] > 300:
		return
...
func _pre_hold(ws: WorldState) -> void:
	if ws.factions[1].support < 1500 and _mod6_active(ws):
		ws.factions[1].support = 1500


## 胜负分支（Event1.cs:207-227/298-318/401-421）。opt0 用<=1500、opt1/2 用<1500。
func _resolve_branch(ws: WorldState, d: Array, opt0: bool) -> String:
	...
	GameManager.queue_ending_after_event(1)   # 原 data[35]=5「人民的选择」(语义映射)
	return "我们不止失去了议会多数，还没能赢得过半议席！真是耻辱！"


func _mod6_active(ws: WorldState) -> bool:
...
func _seats_report(ws: WorldState, suffix: String) -> String:
