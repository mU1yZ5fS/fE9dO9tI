...
##   A = (data[90]==1 || data[90]==2) && (!event_done[503] || resultOfEvents[503]!=0)
...
func evaluate(world: WorldState) -> bool:
	...
	if data.size() <= W.I_YEAR:
		return false
	var date_ok := (data[W.I_MONTH] >= 9 and data[W.I_YEAR] >= 1982) or data[W.I_YEAR] >= 1983
	if not date_ok:
		return false
	# 分支 A
	...
	if data.size() > W.I_MAO_HISTORY_LINE:
		var mhl := data[W.I_MAO_HISTORY_LINE]
