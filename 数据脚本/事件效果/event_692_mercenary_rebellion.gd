...
## 差异：load_scene_after_click + data[35]=14 + LoadScene("Ending")
##   → d[I_ENDING_ROUTE]=14 + GameManager.queue_ending_after_event(14)（项目既有惯例）。

const TXT_TITLE := "勿忘1204年，勿忘马基雅维利教诲"
...
func execute(context: Dictionary) -> void:
	...
	# 原版 load_scene_after_click：点击结果后 data[35]=14 并进入 Ending。
	if d.size() > W.I_ENDING_ROUTE:
		d[W.I_ENDING_ROUTE] = 14
	GameManager.queue_ending_after_event(14)
