...
## 原作 Event520.cs：水下没有龙宫（3选项）。 ## 触发来源见 .tres 与脚本头。文本逐字对齐原版（去空格/ 换行/剥 color）。 ## 选项显隐由 prepare 动态改写；result_text 由本脚本动态生成。

const TXT_R0_A := "event.script.event_520_no_dragon_palace_underwater.c0"
const TXT_R1_A := "event.script.event_520_no_dragon_palace_underwater.c1"
const TXT_R2_A := "event.script.event_520_no_dragon_palace_underwater.c2"
...
# ══════════════════════════════════════════════════════════ # 自动迁移的事件定义 —— 源： 场景/事件界面/events/event_520_no_dragon_palace_underwater.tres # 文案不在本文件，见 资产/本地化/events_zh_CN.csv # ══════════════════════════════════════════════════════════
const META := {
	"id": "event_520",
	"num": 520,
	"priority": 52000,
	"notify": false,
	"display_script": "res://数据脚本/事件效果/event_520_no_dragon_palace_underwater.gd",
	"trigger": [{"t": "ALL", "c": [{"t": "PREV_EVENT_DONE", "ref": "event_519"}, {"t": "RESOURCE_AT_LEAST", "key": "industry", "v": 1000}, {"t": "TECH_UNLOCKED", "v": 21}, {"t": "DATE_AFTER", "key": "1981.4.1"}]}],
