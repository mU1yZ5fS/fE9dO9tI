...
func execute(context: Dictionary) -> void:
	...
	match str(context.get("event_id", "")):
		"pan_arabism": _event_64(ws, option_index)
		"moscow_olympics": _event_65(ws, option_index, context)
		"death_of_tito": _event_66(ws, option_index)
	ws.clamp_empire_relations()
	_sync_empire_mirrors(ws)


func _event_64(ws: WorldState, option_index: int) -> void:
	if option_index == 0:
		_add_empire_power(ws, EmpireData.USA, 10)
		return
	_add_data(ws, {W.I_BUDGET: -70, W.I_AGENTS: -50, W.I_DIPLO: 10,
	...
	_add_faction_ideology(ws, {FactionData.MODERATE: 24, FactionData.REFORMIST: 24})
	_change_politicians(ws, {1: [100, 120], 2: [100, 120]})


func _event_65(ws: WorldState, option_index: int, context: Dictionary) -> void:
	match option_index:
		0:
			_add_data(ws, {W.I_PARTY_SUPPORT: 150, W.I_PEOPLE_SUPPORT: 80,
		...
		2:
			...
			_add_empire_relation(ws, EmpireData.USSR, 50)
		3:
			_add_data(ws, {W.I_PARTY_SUPPORT: 70, W.I_PEOPLE_SUPPORT: 30,
		...
		4:
			_add_data(ws, {W.I_PARTY_SUPPORT: 200, W.I_PEOPLE_SUPPORT: 50,
...
func _event_66(ws: WorldState, option_index: int) -> void:
	match option_index:
		0:
			ws.数值表[W.I_DIPLO] -= 10
			_add_empire_relation(ws, EmpireData.USA, 20)
			_add_empire_relation(ws, EmpireData.USSR, 20)
		1:
			_add_data(ws, {W.I_PARTY_SUPPORT: -50, W.I_DIPLO: -20})
			_add_empire_relation(ws, EmpireData.USA, 50)
			_add_empire_relation(ws, EmpireData.USSR, 50)
		...
		2:
			_add_data(ws, {W.I_PARTY_SUPPORT: -30, W.I_DIPLO: -15})
			_add_empire_relation(ws, EmpireData.USSR, 30)
