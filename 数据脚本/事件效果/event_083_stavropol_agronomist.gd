...
##  - result0（泄露情报）：data[1]+=50 党内支持、data[9]-=50 特工、苏 leaders[3](安德罗波夫).support-=1、data[149]=1
##  - result1（媒体揭露）：苏关系>=500 → 同上（data[8]-=20 预算 而非特工）；否则仅 data[8]-=20
##  - result2（留到将来）：无效果
## 选项条件：
...
func execute(context: Dictionary) -> void:
	...
	if opt == 0:
		...
		_leader_support(-1)
		if d.size() > 149:
			d[149] = 1   # 原 data[149]（中苏关系路线，无端口命名键）
