...
const TXT_TITLE := "黄金海岸的再革命？——第三幕"
const TXT_DESC := "两年以来，罗林斯将社会主义、民族主义和民粹主义混合起来的理念的实施并未能为加纳经济带来十分乐观的发展，政府补贴为财政带来了负担并造成了通货膨胀。有报告显示，罗林斯正在寻求外部的经济援助和投资，苏东阵营甚至是西方控制的IMF都将可能成为加纳的帮助者。很明显，现在正是我们掺和加纳的时候，应不应该为罗林斯提供一笔无息贷款？这将能为我们争取一个盟友，并保住非洲的一个进步政权。"
const TXT_OPT0 := "我们将为加纳提供帮助"
const TXT_OPT0_DIS := "我们不会帮他"
const TXT_OPT1 := "邀请加纳加入经互会，在经互会体系内为加纳提供帮助"
...
func execute(context: Dictionary) -> void:
	...
	match opt:
		0:
			...
			if c63 != null: c63.government = 1
			if c63 != null: c63.sub_government = 1
		...
		2:
			...
			if ws.empires[0].power > ws.empires[1].power:
				...
				if c63 != null: c63.sub_government = 15
				return
