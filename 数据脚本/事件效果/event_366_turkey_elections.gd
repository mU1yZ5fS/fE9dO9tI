...
const TXT_OPT0 := "event.script.event_366_turkey_elections.c0"
const TXT_OPT0_DIS := "event.script.event_366_turkey_elections.c1"
const TXT_OPT1 := "event.script.event_366_turkey_elections.c2"
const TXT_OPT1_DIS := "event.script.event_366_turkey_elections.c3"
const TXT_OPT2 := "event.script.event_366_turkey_elections.c4"
const TXT_OPT0_DIS_BUDGET := "event.script.event_366_turkey_elections.c5"
const TXT_OPT0_DIS_AGENTS := "event.script.event_366_turkey_elections.c6"
const TXT_OPT1_DIS_BUDGET := "event.script.event_366_turkey_elections.c7"
const TXT_OPT1_DIS_AGENTS := "event.script.event_366_turkey_elections.c8"

const TXT_R0 := "event.script.event_366_turkey_elections.c9"
const TXT_R1 := "event.script.event_366_turkey_elections.c10"
const TXT_R2 := "event.script.event_366_turkey_elections.c11"
...
func prepare(event_def: EventDef, world: WorldState) -> void:
	...
	if agents > 30 and _budget_reserve(world) >= 50:
		...
		if _budget_reserve(world) < 50:
			_disable(opt[0], tr(TXT_OPT0_DIS_BUDGET))
		else:
			_disable(opt[0], tr(TXT_OPT0_DIS_AGENTS))
	if agents > 30 and _budget_reserve(world) >= 50:
		...
		if _budget_reserve(world) < 50:
			...
			_disable(opt[1], tr(TXT_OPT1_DIS_AGENTS))
...
# ══════════════════════════════════════════════════════════
# 自动迁移的事件定义 —— 源： 场景/事件界面/events/event_366_turkey_elections.tres
...
const META := {
	...
	"priority": 36600,
	"notify": false,
	"display_script": "res://数据脚本/事件效果/event_366_turkey_elections.gd",
	"trigger": [{"t": "DATE_AFTER", "key": "1977.4.21"}],
