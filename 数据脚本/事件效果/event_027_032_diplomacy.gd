...
func execute(context: Dictionary) -> void:
	...
	match str(context.get("event_id", "")):
		"hong_kong_macau": _event_27(ws, option_index, context)
		"indonesia_after_suharto": _event_28(ws, option_index)
		"pressure_north_korea": _event_29(ws, option_index, context)
		"palestine_settlement": _event_30(ws, option_index, context)
		"south_korea_election": _event_31(ws, option_index)
		"mongolia_reform": _event_32(ws, option_index)
	ws.clamp_empire_relations()
	_sync_empire_mirrors(ws)


func _event_27(ws: WorldState, option_index: int, context: Dictionary) -> void:
	var d := ws.数值表
	match option_index:
		0:
		...
		1:
			if (d[W.I_DIPLO] <= 60 or d[W.I_INFLUENCE] >= 150) and _relation(ws, EmpireData.USA) >= 800:
		...
		2:
			if (d[W.I_DIPLO] <= 50 or d[W.I_INFLUENCE] >= 250) and _relation(ws, EmpireData.USA) >= 800:
...
func _event_28(ws: WorldState, option_index: int) -> void:
	var indonesia := ws.get_country_by_legacy_index(50)
	match option_index:
		0:
			_add_power(ws, EmpireData.USA, -20)
			_set_country(indonesia, 3, false, false, false)
		1:
			_add_power(ws, EmpireData.USA, -40)
			ws.数值表[W.I_INFLUENCE] += 20
			_set_country(indonesia, 2, false, true, false)
			_change_loyalty(ws, func(personality: int) -> bool: return personality <= 2, 70)
		2:
			_add_power(ws, EmpireData.USA, -50)
			_add_data(ws.数值表, {W.I_INFLUENCE: 40, W.I_DIPLO: 20})
			_set_country(indonesia, 1, false, true, true)
			_change_loyalty(ws, func(personality: int) -> bool: return personality <= 1, 100)


func _event_29(ws: WorldState, option_index: int, context: Dictionary) -> void:
	var d := ws.数值表
	var north_korea := ws.get_country_by_legacy_index(10)
	match option_index:
		0:
			d[W.I_INFLUENCE] += 10
			_add_relation(ws, EmpireData.USA, 70)
			_change_loyalty_split(ws, 1, -50, 50)
		1:
			if d[W.I_INFLUENCE] > _power(ws, EmpireData.USSR):
				d[W.I_INFLUENCE] += 10
				_add_relation(ws, EmpireData.USA, 100)
				_add_relation(ws, EmpireData.USSR, -100)
				_add_power(ws, EmpireData.USA, 30)
				_change_loyalty_split(ws, 1, -100, 100)
				if north_korea != null:
					north_korea.government = 2
			else:
				_set_korea_soviet_result(context)
				_apply_korea_soviet_turn(ws, north_korea)
		2:
			d[W.I_INFLUENCE] += 10
			d[W.I_BUDGET] += 40
			_change_loyalty(ws, func(personality: int) -> bool: return personality >= 1, 100)
...
func _event_30(ws: WorldState, option_index: int, context: Dictionary) -> void:
	var d := ws.数值表
	...
	match option_index:
		0:
			if d[W.I_INFLUENCE] >= 150:
				d[W.I_INFLUENCE] += 10
				_add_relation(ws, EmpireData.USSR, 100)
				_add_relation(ws, EmpireData.USA, -50)
				d[W.I_PALESTINE_STATUS] = 2
				if israel != null: israel.set_tag("亲美", false)
			else:
				_apply_peace_failure(ws, context)
		1:
			d[W.I_INFLUENCE] += 10
			_add_relation(ws, EmpireData.USSR, 50)
			_add_relation(ws, EmpireData.USA, 50)
...
func _event_31(ws: WorldState, option_index: int) -> void:
	...
	match option_index:
		...
		2:
			...
			_add_relation(ws, EmpireData.USA, -250)
			if south_korea != null:
				south_korea.set_tag("亲美", false)
				south_korea.government = 2


func _event_32(ws: WorldState, option_index: int) -> void:
	var mongolia := ws.get_country_by_legacy_index(9)
	match option_index:
		0:
			_add_power(ws, EmpireData.USSR, 10)
			_add_relation(ws, EmpireData.USSR, -100)
		1:
...
func _relation(ws: WorldState, empire_index: int) -> int:
	return ws.empires[empire_index].relations if empire_index >= 0 and empire_index < ws.empires.size() else 0


func _power(ws: WorldState, empire_index: int) -> int:
	return ws.empires[empire_index].power if empire_index >= 0 and empire_index < ws.empires.size() else 0


func _add_relation(ws: WorldState, empire_index: int, delta: int) -> void:
	if empire_index >= 0 and empire_index < ws.empires.size():
		ws.empires[empire_index].relations += delta


func _add_power(ws: WorldState, empire_index: int, delta: int) -> void:
	if empire_index >= 0 and empire_index < ws.empires.size():
		ws.empires[empire_index].power += delta


func _change_loyalty(ws: WorldState, predicate: Callable, delta: int) -> void:
...
func _change_loyalty_split(ws: WorldState, threshold: int, low_delta: int, high_delta: int) -> void:
	for politician in ws.politicians:
		if politician != null:
...
func _apply_korea_soviet_turn(ws: WorldState, north_korea: CountryData) -> void:
	_add_power(ws, EmpireData.USSR, 20)
...
func _apply_peace_failure(ws: WorldState, context: Dictionary) -> void:
