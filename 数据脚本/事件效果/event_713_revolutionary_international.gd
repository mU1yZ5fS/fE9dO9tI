...
func _chinese_sub_government() -> int:
	...
	if china.government == GameConstants.Government.AUTHORITARIAN:
		if _event_result("event_674") == 2:
			...
			result = 0
		elif (data.econ_system >= 13 and data.war_support >= 700 and not _mod_active(6)) \
				or _mod_active(38):
			result = 9
		elif data.econ_system <= 13 and data.war_support >= 700 \
				and data.diplomatic_reputation >= 700 and (_mod_active(6) or _mod_active(3)):
			result = 10
		...
		if _mod_active(49):
			result = 18
		elif _mod_active(6) and _mod_active(3) and data.party_system <= 7 \
		...
		if _mod_active(40):
			...
			result = 3
				...
				and data.war_support < 700 and not _mod_active(3) \
				and data.press_policy >= 17:
			result = 21
		else:
			result = 15
