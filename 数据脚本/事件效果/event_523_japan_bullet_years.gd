...
## 原作 Event523.cs：日本的铅弹岁月（2选项）。 ## 触发来源见 .tres 与脚本头。文本逐字对齐原版（去空格/ 换行/剥 color）。 ## 选项显隐由 prepare 动态改写；result_text 由本脚本动态生成。

const TXT_OPT0_DIS := "event.script.event_523_japan_bullet_years.c0"
const TXT_R0_A := "event.script.event_523_japan_bullet_years.c1"
const TXT_R1_A := "event.script.event_523_japan_bullet_years.c2"
...
# ══════════════════════════════════════════════════════════ # 自动迁移的事件定义 —— 源： 场景/事件界面/events/event_523_japan_bullet_years.tres # 文案不在本文件，见 资产/本地化/events_zh_CN.csv # ══════════════════════════════════════════════════════════
const META := {
	"id": "event_523",
	"num": 523,
	"priority": 52300,
	"notify": false,
	"display_script": "res://数据脚本/事件效果/event_523_japan_bullet_years.gd",
	"trigger": [{"t": "ALL", "c": [{"t": "COUNTRY_FIELD_AT_MOST", "key": "puppet_of", "v": -1, "target": "44"}, {"t": "DATE_AFTER", "key": "1977.6.1"}]}],
