...
## 原作 Event118.cs：“和平之国达鲁萨兰”的再回（文莱独立，单选项）。 ## 触发：TimeScript.cs:10836-10842 —— ##   ((日>=9 且 月>=1 且 年>=1984) (月>=2 且 年>=1984) 年>=1985) ##   && !c49.parts[0]（已由本脚本 evaluate 实现，见文件底部；c49=马来西亚原版序号）。 ## 效果：c111.isASEAN=true（→ set_tag("asean", true)）、Gosstroy=0、SubGosstroy=13、 ##   USA power+=20。选项文本取自 new_events_text[1433]“密切关注......”。

const TXT_RESULT := "event.script.event_118_brunei.c0"
...
## TimeScript.cs:10838 触发守卫：!(c49.parts[0])。 ## c49=马来西亚（原版序号49）。parts 为空时 parts[0] 视为 false，事件照常触发。
...
# ══════════════════════════════════════════════════════════ # 自动迁移的事件定义 —— 源： 场景/事件界面/events/event_118_brunei.tres # 文案不在本文件，见 资产/本地化/events_zh_CN.csv # ══════════════════════════════════════════════════════════
const META := {
	"id": "event_118",
	"num": 118,
	"priority": 1180,
	"notify": false,
	"trigger_script": "res://数据脚本/事件效果/event_118_brunei.gd",
