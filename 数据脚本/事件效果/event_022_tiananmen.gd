...
## 原作 Event22.cs：天安门事件（三选项，选项1/2 按 data.democracy_movement 动态显隐）。
## 触发：TimeScript.cs:10187 —— (日>=5 且 月>=4 且 年>=1976) || (月>=5 且 年>=1976) || 年>=1977，
##   端口为 DATE_AFTER "1976.4.5"。
## 差异：
##  - 原版按钮 1/2 在 data.democracy_movement 不满足时 Destroy(button) 并替换为“人民不想离去！”，
##    端口为 prepare 动态 _disable（enable_condition=RESOURCE_AT_LEAST party_system 99999）。
##  - data.democracy_movement 无端口命名键，raw index + 注释。
