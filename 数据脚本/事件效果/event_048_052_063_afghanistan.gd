...
func execute(context: Dictionary) -> void:
	...
	match str(context.get("event_id", "")):
		...
		"afghan_civil_war": _event_50(option_index)
...
func _event_48(option_index: int) -> void:
	var afghanistan := ws.get_country_by_legacy_index(12)
	if afghanistan != null:
...
func _event_49(option_index: int, context: Dictionary) -> void:
	...
	match option_index:
		...
		1:
			...
			_start_afghan_war("DRA", "Mujahideen", 500, 500, 1, -1, true)


func _event_50(option_index: int) -> void:
	var afghanistan := ws.get_country_by_legacy_index(12)
	if afghanistan != null: afghanistan.set_tag("亲美", false)
	match option_index:
		0:
			_start_afghan_war("DRA", "Mujahideen", 750, 250, 1, 0, true)
		1:
			_add_data({W.I_PARTY_SUPPORT: 50, W.I_INFLUENCE: 10,
