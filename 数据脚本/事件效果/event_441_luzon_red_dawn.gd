...
const TXT_TITLE := "吕宋群岛的赤色黎明！"

const TXT_DESC := "在我们的帮助下，菲律宾共产党成功夺取了全菲律宾政权：菲共的59式坦克开进马拉卡南宫（总统府），活捉了试图逃离的马科斯之后，伪菲律宾政府宣告投降。菲共和其他左翼政党组成的菲律宾全国民主阵线建立了菲律宾人民民主共和国。"

const TXT_OPT0 := "庆祝菲律宾同志们的胜利！"
