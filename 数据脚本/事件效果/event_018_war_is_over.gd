...
func prepare(event_def: EventDef, ws: WorldState) -> void:
	var d := ws.数值表
	var war_id: int = d[W.I_WAR_RESOLVE] if d.size() > W.I_WAR_RESOLVE else -1
	var war := _get_war(ws, war_id)
	var war_name := war.name_war if war != null else ""

	# TextOfEvents 分支
	if _is_mongol_defeat(ws, war_id, war):
		event_def.title = "真该死！"
		event_def.description = "在长时间的血腥冲突之后“%s”终于结束了。\n前线的状态并不乐观……" % war_name
	elif _is_japan_revolution(ws, war_id, war):
	...
	elif _is_kefir_doom(ws, war_id, war):
	...
	if war_id < 8:
		button_text = "和平万岁！"
	elif _is_mongol_defeat(ws, war_id, war):
		button_text = "发生什么了？"
	elif _is_japan_revolution(ws, war_id, war):
...
func execute(context: Dictionary) -> void:
	...
	elif _is_ussr_victory(ws, war_id, war):
		context["result_title"] = "真该死！"
...
func _get_war(ws: WorldState, war_id: int) -> WarData:
	if war_id >= 0 and war_id < ws.wars.size():
		return ws.wars[war_id]
	return null


func _is_mongol_defeat(ws: WorldState, war_id: int, war: WarData) -> bool:
	if war_id != 69 and war_id != 70:
		return false
	return war != null and war.infl1 < 1000


func _is_japan_revolution(ws: WorldState, war_id: int, war: WarData) -> bool:
	return war_id == 36 and war != null and war.infl2 >= 1000


func _is_kefir_doom(ws: WorldState, war_id: int, war: WarData) -> bool:
...
func _is_ussr_victory(ws: WorldState, war_id: int, war: WarData) -> bool:
