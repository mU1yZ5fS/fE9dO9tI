...
func execute(context: Dictionary) -> void:
	...
	d[W.I_STABILITY] = 100
	_install_hua_guofeng(ws, d)

	if opt == 0:
		d[W.I_THOUGHT_FREEDOM] -= 50
		d[W.I_PEOPLE_SUPPORT] += 20
		_add_loyalty_1_to_4(ws, 200)
...
func _install_hua_guofeng(ws: WorldState, d: Array) -> void:
...
func _add_loyalty_1_to_4(ws: WorldState, delta: int) -> void:
	for i in range(1, 5):
