extends "res://数据脚本/event_script_base.gd"

## 原作 Event346.cs：在军委会中任职的平民。触发：ReqEventForDLC02.cs:612-615 —— 日期>=1981.6.11。




const TXT_R0 := "event.script.event_346_civilians_in_military.c0"
const TXT_R1 := "event.script.event_346_civilians_in_military.c1"

func execute(context: Dictionary) -> void:
	if not _bind_world():
		return
	var opt := int(context.get("option_index", -1))
	match opt:
		0:
			_add(W.I_PARTY_SUPPORT, 50)
			_add(W.I_THOUGHT_FREEDOM, 50)
			context["result_text"] = tr(TXT_R0)
		1:
			_add(W.I_PEOPLE_SUPPORT, 30)
			_add(W.I_BUDGET, 50)
			_add(W.I_ARMY, -250)
			_add(W.I_PARTY_SUPPORT, -100)
			_add(W.I_DIPLO, -20)
			_add(W.I_LIVING, 25)
			if d.size() > W.I_MIL_DOCTRINE and d.military_doctrine < 33:
				_add(W.I_MIL_DOCTRINE, 1)
			_set_modifier_active(33)
			context["result_text"] = tr(TXT_R1)





func _set_data(index: int, value: int) -> void:
	if d.size() > index:
		d.set_data_by_index(index, value)






func _modifier_active(index: int) -> bool:
	return ws.modifiers.size() > index and ws.modifiers[index] != null and ws.modifiers[index].is_active


func _set_modifier_active(index: int) -> void:
	if ws.modifiers.size() > index and ws.modifiers[index] != null:
		ws.modifiers[index].is_active = true



# ══════════════════════════════════════════════════════════
# 自动迁移的事件定义 —— 源： 场景/事件界面/events/event_346_civilians_in_military.tres
# 文案不在本文件，见 资产/本地化/events_zh_CN.csv
# ══════════════════════════════════════════════════════════
const META := {
	"id": "event_346",
	"num": 346,
	"priority": 34600,
	"notify": false,
	"display_script": "res://数据脚本/事件效果/event_346_civilians_in_military.gd",
	"trigger": [{"t": "DATE_AFTER", "key": "1981.6.11"}],
	"options": [{"fx": [{"t": "CUSTOM_SCRIPT"}]}, {"fx": [{"t": "CUSTOM_SCRIPT"}]}],
}
