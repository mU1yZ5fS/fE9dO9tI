...
func execute(context: Dictionary) -> void:
	...
	if opt == 0:
		_option_debate(ws, d, context)
	elif opt == 1:
		d[W.I_PEOPLE_SUPPORT] -= 50
		d[W.I_AGENTS] -= 100
		_party_support_segmented(d)
		_purge_politicians(ws, -200)
...
func _option_debate(ws: WorldState, d: Array, context: Dictionary) -> void:
	var loyal_count := 0
...
func _party_support_segmented(d: Array) -> void:
...
func _purge_politicians(ws: WorldState, loyalty_delta: int) -> void:
	for p in ws.politicians:
		if p == null:
			continue
		var a := (
