...
func _result_1(context: Dictionary) -> void:
	...
	if _socialist_count() >= 3 and greece != null and greece.government == 2:
		if cyprus != null:
			_leave_alliances(cyprus)
			cyprus.government = 1
			cyprus.sub_government = 16
	...
	elif greece != null and greece.government != 2:
		if cyprus != null:
			_leave_alliances(cyprus)
			cyprus.government = 3
			cyprus.sub_government = 12
