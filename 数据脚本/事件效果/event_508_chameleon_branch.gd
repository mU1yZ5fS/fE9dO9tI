...
const TXT_TITLE := "树枝不会在变色龙手里折断"
const TXT_DESC := "随着克雷库政权走社会主义道路以来已经过去了十余年。但贝宁的一切都说不上算多好，过于急切的追求工业化导致了经济发展的失衡。而公开反对法帝国主义则成为了压垮骆驼的最后一根稻草克雷库多次表示希望西方国家能帮助贝宁度过难关。他首先强调“贝宁革命的最终目的是社会主义，但社会主义要分阶段实现。”克雷库政权对小资产阶级和外国资本的态度也有了显著的转变。从一开始的完全打压与限制，再到仅在国家层面进行监管，最后则是开始准备扭转国有化进程。很明显，贝宁革命正在危机边缘，领导人同志，我们也许可以提供一笔无息贷款？这会为我们带来一个革命同志，同时狠狠的打苏联的脸。"
const TXT_OPT0 := "扭转贝宁右转的方向！（需要8百万预算）"
const TXT_OPT0_DIS := "这是在做无用功"
const TXT_OPT1 := "将贝宁纳入经济互助委员会体系"
...
func prepare(event_def: EventDef, world: WorldState) -> void:
	...
	if ws.数值表[56] <= 2 and ws.influence_prc >= 600:
...
func execute(context: Dictionary) -> void:
	...
	match opt:
		0:
			...
			if ws.数值表[131] == 2  and  ws.get_flag("YugAgree"):
			...
			if c62 != null: c62.government = 1
			if c62 != null: c62.sub_government = 17
			...
			if ws.数值表[131] == 2  and  ws.get_flag("YugAgree"):
				...
				if c62 != null: c62.sub_government = 3
		1:
			context["result_text"] = TXT_R1_A
			_add(8, -(10))
			if c62 != null: c62.government = 1
			if c62 != null: c62.sub_government = 1
			if c62 != null: c62.set_tag("对华贸易", true)
			_add_relation(1, 100)
			_add_power(1, 20)
