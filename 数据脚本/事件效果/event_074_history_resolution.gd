...
## 选项：0/1/2 按 data[90]（毛历史评价路线）选择对应版本通过；3=重写（需路线<2 一党制<8 或多党联盟>66%）；4=不通过。
...
##  - result3（重写）：data[56]<2 → 党内+20/民众+50/改革动量-20/国际声望-60/思想自由+40、data[90]=0、政客（0派+100p+150l；1派-50p-50l；2派-100p-100l；3派-150p-200l）
##               data[56]==1||2 → 党内+80/民众+40/改革动量+10/国际声望-50/思想自由-20、data[90]=1、政客（同 result1）
##               data[56]>2 → 党内-100/民众-150/兵源-200/改革动量+40/国际声望+200/思想自由-50、影响力-30、data[90]=2、政客（0/1/2派-300l；3派+150l）
##  - result4（不通过）：无效果
## 差异：party_change 派系缓冲端口无等价 → 跳过；result2 的 load_scene → start_event("congress_conspiracy")
## 索引映射修正（2026-08-04 随批审查）：原版 data[4]=思想自由→I_THOUGHT_FREEDOM、data[6]=国际声望→I_DIPLO。
...
func _add_data(party: int, people: int, freedom: int, momentum: int, diplo: int) -> void:
	if d.size() > W.I_PARTY_SUPPORT:
		d[W.I_PARTY_SUPPORT] += party
	if d.size() > W.I_PEOPLE_SUPPORT:
		d[W.I_PEOPLE_SUPPORT] += people
	if d.size() > W.I_THOUGHT_FREEDOM:
		d[W.I_THOUGHT_FREEDOM] += freedom
	if d.size() > W.I_REFORM_MOMENTUM:
		d[W.I_REFORM_MOMENTUM] += momentum
	if d.size() > W.I_DIPLO:
		d[W.I_DIPLO] += diplo
