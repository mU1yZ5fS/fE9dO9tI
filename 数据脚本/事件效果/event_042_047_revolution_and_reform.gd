...
func execute(context: Dictionary) -> void:
	...
	match str(context.get("event_id", "")):
		"iranian_revolution": _event_42(ws, option_index)
		"vietnam_cmea": _event_43(ws, option_index)
		"reformers_take_power": _event_44(ws, option_index)
		"reform_and_openness": _event_45(ws)
		"hungarian_crisis": _event_46(ws, option_index)
		"beijing_spring": _event_47(ws, option_index)
	ws.clamp_empire_relations()
	_sync_empire_mirrors(ws)


func _event_42(ws: WorldState, option_index: int) -> void:
	...
	match option_index:
		...
		1:
			_add_data(ws, {W.I_IRAN_LEFT_SUPPORT: 70, W.I_AGENTS: -50, W.I_DIPLO: 20})
			if iran != null: iran.development = 1
		2:
			_add_data(ws, {W.I_IRAN_ISLAMIST_SUPPORT: 70, W.I_AGENTS: -50, W.I_DIPLO: 20})
			if iran != null: iran.development = 3
		3:
			_add_data(ws, {W.I_IRAN_SHAH_SUPPORT: 70, W.I_AGENTS: -50, W.I_DIPLO: -10})
			if iran != null: iran.development = 0
		4:
			_add_data(ws, {W.I_IRAN_DEMOCRAT_SUPPORT: 70, W.I_AGENTS: -50, W.I_DIPLO: -20})
			if iran != null: iran.development = 2


func _event_43(ws: WorldState, option_index: int) -> void:
	...
	match option_index:
		...
		1:
			_add_data(ws, {W.I_PARTY_SUPPORT: 100, W.I_AGENTS: -30})
			_add_empire_relation(ws, EmpireData.USSR, -70)


func _event_44(ws: WorldState, option_index: int) -> void:
	match option_index:
		0:
			_apply_market_reform_base(ws)
			var reform_leader := _faction_leader_index(ws, FactionData.REFORMIST)
			if reform_leader >= 0:
				var transition = POLITICAL_TRANSITION.new()
				transition._swap_leader_with_politician(ws, reform_leader, FactionData.REFORMIST)
			# 原作在身份互换后才按新 traits 结算权力变化。
			_change_politicians(ws, {0: [0, -200], 1: [0, 100], 2: [0, 200], 3: [0, 80]})
		1:
			_add_data(ws, {W.I_PARTY_SUPPORT: -150, W.I_PEOPLE_SUPPORT: -120,
				W.I_AGENTS: -150, W.I_THOUGHT_FREEDOM: 150, W.I_DIPLO: 30})
		2:
			_apply_market_reform_base(ws)
			_change_politicians(ws, {0: [-250, -200], 1: [100, 100],
				2: [200, 200], 3: [70, 80]})
	GameManager._sync_in_power_flags(ws)


func _event_45(ws: WorldState) -> void:
	_add_data(ws, {W.I_INFLUENCE: -20, W.I_THOUGHT_FREEDOM: 50,
	...
	_change_politicians(ws, {1: [0, 50], 2: [0, 100], 3: [0, 50]})


func _event_46(ws: WorldState, option_index: int) -> void:
	var hungary := ws.get_country_by_legacy_index(4)
	match option_index:
		0:
			_add_empire_power(ws, EmpireData.USSR, -10)
		1:
			_add_data(ws, {W.I_INFLUENCE: 20, W.I_AGENTS: -80, W.I_DIPLO: 20})
			_add_empire_power(ws, EmpireData.USSR, -20)
			_add_empire_relation(ws, EmpireData.USSR, -300)
			if hungary != null:
				hungary.government = 1
				hungary.set_tag("对华贸易", true)
				hungary.set_tag("亲苏", false)
		2:
			_add_data(ws, {W.I_PARTY_SUPPORT: -100, W.I_INFLUENCE: -15,
...
func _event_47(ws: WorldState, option_index: int) -> void:
	match option_index:
		0:
			_add_data(ws, {W.I_PARTY_SUPPORT: -80, W.I_THOUGHT_FREEDOM: 100,
				W.I_PEOPLE_SUPPORT: -80, W.I_DIPLO: 10})
			_subtract_faction_fraction(ws, FactionData.REFORMIST, 0.05)
			_change_politicians(ws, {2: [-100, -100]})
		1:
			_add_data(ws, {W.I_PARTY_SUPPORT: 30, W.I_THOUGHT_FREEDOM: 80,
				W.I_PEOPLE_SUPPORT: -50, W.I_DIPLO: -10})
			_change_politicians(ws, {2: [50, 50]})
		2:
			_add_data(ws, {W.I_PARTY_SUPPORT: -80, W.I_THOUGHT_FREEDOM: 120,
				W.I_PEOPLE_SUPPORT: -60})
			_add_faction_ideology(ws, {FactionData.REFORMIST: 100})
			_change_politicians(ws, {2: [50, 150]})


func _apply_market_reform_base(ws: WorldState) -> void:
	...
	_subtract_faction_fraction(ws, FactionData.MAOIST, 0.15)
	_subtract_faction_fraction(ws, FactionData.CONSERVATIVE, 0.15)


func _faction_leader_index(ws: WorldState, faction_index: int) -> int:
