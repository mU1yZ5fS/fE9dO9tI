...
## 原作 Event507.cs：“虾”行动（3选项）。 ## 触发来源见 .tres 与脚本头。文本逐字对齐原版（去空格/ 换行/剥 color）。 ## 选项显隐由 prepare 动态改写；result_text 由本脚本动态生成。

const TXT_OPT0_DIS := "event.script.event_507_operation_shrimp.c0"
const TXT_OPT1_DIS := "event.script.event_507_operation_shrimp.c1"
const TXT_R0_A := "event.script.event_507_operation_shrimp.c2"
const TXT_R1_A := "event.script.event_507_operation_shrimp.c3"
const TXT_R2_A := "event.script.event_507_operation_shrimp.c4"
...
func prepare(event_def: EventDef, world: WorldState) -> void:
	...
	if ws.political_line <= 2 and ws.influence_prc >= 10:
...
# ══════════════════════════════════════════════════════════ # 自动迁移的事件定义 —— 源： 场景/事件界面/events/event_507_operation_shrimp.tres # 文案不在本文件，见 资产/本地化/events_zh_CN.csv # ══════════════════════════════════════════════════════════
const META := {
	"id": "event_507",
	"num": 507,
	"priority": 50700,
	"notify": false,
	"display_script": "res://数据脚本/事件效果/event_507_operation_shrimp.gd",
	"trigger": [{"t": "DATE_AFTER", "key": "1977.1.15"}],
