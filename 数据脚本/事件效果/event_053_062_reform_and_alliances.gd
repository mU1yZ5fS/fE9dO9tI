...
func execute(context: Dictionary) -> void:
	...
	match str(context.get("event_id", "")):
		...
		"reform_investment": _event_54(option_index)
		"burmese_socialism": _event_55(option_index)
		...
		"economic_union": _event_59(option_index)
		"military_alliance": _event_60(option_index)
		"anthem_problem": _event_61(option_index)
		"inner_mongolia_problem": _event_62(option_index)
...
func _event_54(option_index: int) -> void:
	match option_index:
		0:
		...
		2:
			...
			_add_faction_ideology({FactionData.REFORMIST: 50, FactionData.LIBERAL: 80})
			_subtract_faction_fraction(FactionData.MODERATE, 0.09)
			ws.set_flag("sez", true)
			_change_politicians({
...
func _event_55(option_index: int) -> void:
	...
	match option_index:
		...
		2:
			_add_data({W.I_AGENTS: -40, W.I_DIPLO: 20, W.I_BUDGET: -20})
...
func _event_59(option_index: int) -> void:
...
func _event_60(option_index: int) -> void:
	if option_index != 0:
		return
...
func _event_61(option_index: int) -> void:
	match option_index:
		0:
			_add_data({W.I_BUDGET: -10, W.I_PEOPLE_SUPPORT: 20, W.I_PARTY_SUPPORT: 70})
			_change_politicians({1: [0, 50], 2: [0, 50]})
		1:
			_add_data({W.I_PARTY_SUPPORT: -100, W.I_DIPLO: 10,
				W.I_PEOPLE_SUPPORT: 40})
			_add_empire_relation(EmpireData.USA, -50)
			_add_empire_relation(EmpireData.USSR, -50)
			_change_politicians({0: [0, 50]})
		2:
			_add_data({W.I_PARTY_SUPPORT: -50, W.I_DIPLO: 5,
				W.I_BUDGET: -10, W.I_PEOPLE_SUPPORT: 10})


func _event_62(option_index: int) -> void:
	match option_index:
		0:
			_add_data({W.I_PARTY_SUPPORT: -80, W.I_MANPOWER: 50,
				W.I_THOUGHT_FREEDOM: 20, W.I_PEOPLE_SUPPORT: 60, W.I_DIPLO: -10,
				W.I_REFORM_MOMENTUM: 10, W.I_BUDGET: -30, W.I_COMMUNICATIONS: 20})
			_add_empire_relation(EmpireData.USA, 30)
			_add_empire_relation(EmpireData.USSR, 100)
		1:
			_add_data({W.I_PARTY_SUPPORT: -20, W.I_MANPOWER: -150,
				W.I_PEOPLE_SUPPORT: -50})
		2:
			_add_data({W.I_PARTY_SUPPORT: 10, W.I_MANPOWER: 10,
				W.I_THOUGHT_FREEDOM: 30, W.I_PEOPLE_SUPPORT: 20,
				W.I_DIPLO: -5})
			_add_empire_relation(EmpireData.USA, -50)
		3:
			_add_data({W.I_PARTY_SUPPORT: 30, W.I_BUDGET: -10,
				W.I_PEOPLE_SUPPORT: 30, W.I_DIPLO: -5, W.I_MANPOWER: 30,
				W.I_COMMUNICATIONS: 10})
		4:
			_add_data({W.I_PARTY_SUPPORT: -150, W.I_BUDGET: -60,
...
func _unlock_tech(tech_index: int) -> void:
