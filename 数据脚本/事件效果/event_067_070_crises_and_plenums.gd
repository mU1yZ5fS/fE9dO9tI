...
func execute(context: Dictionary) -> void:
	...
	match str(context.get("event_id", "")):
		...
		"gwangju_uprising": _event_68(option_index)
		"little_gang_of_four": _event_69(option_index)
		"zhou_enlai_heirs": _event_70(option_index)
	...
	match option_index:
		...
		3:
			if _empire_relation(EmpireData.USSR) >= 800:
				...
				_add_data({W.I_INFLUENCE: 10, W.I_DIPLO: 20})
				_add_empire_power(EmpireData.USSR, -20)
				if poland != null:
					poland.government = 0
		...
		4:
			...
			if _empire_relation(EmpireData.USA) >= 80 and united_states != null and united_states.stability == 1:
...
func _event_68(option_index: int) -> void:
	if option_index == 0:
		_add_empire_power(EmpireData.USA, 10)
		return
...
func _event_69(option_index: int) -> void:
	match option_index:
		0:
...
func _event_70(option_index: int) -> void:
	match option_index:
		0:
