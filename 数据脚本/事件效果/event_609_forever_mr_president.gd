...
const TXT_OPT0_DIS := "event.script.event_609_forever_mr_president.c0"
const TXT_OPT1_DIS := "event.script.event_609_forever_mr_president.c1"
const TXT_OPT2_DIS := "event.script.event_609_forever_mr_president.c2"
const TXT_R0_INTRO := "event.script.event_609_forever_mr_president.c3"
const TXT_R0_BODY := "event.script.event_609_forever_mr_president.c4"
const TXT_R0_MAO := "event.script.event_609_forever_mr_president.c5"
const TXT_R0_TAIL := "event.script.event_609_forever_mr_president.c6"
const TXT_NAME_SUCCESS := "event.script.event_609_forever_mr_president.c7"
const TXT_R0_FAIL_BODY := "event.script.event_609_forever_mr_president.c8"
const TXT_NAME_FAIL := "event.script.event_609_forever_mr_president.c9"
const TXT_R1 := "event.script.event_609_forever_mr_president.c10"
const TXT_NAME_R1 := "event.script.event_609_forever_mr_president.c11"
const TXT_R2 := "event.script.event_609_forever_mr_president.c12"
const TXT_R3 := "event.script.event_609_forever_mr_president.c13"
	...
	match opt:
		0:
			...
			if _tech_unlocked(18):
				...
				if comoros != null:
					...
					comoros.chinese_name = tr(TXT_NAME_SUCCESS)
			else:
				text += tr(TXT_R0_FAIL_BODY)
				if comoros != null:
					...
					comoros.chinese_name = tr(TXT_NAME_FAIL)
		...
		1:
			...
			if comoros != null:
				...
				comoros.set_tag("对华贸易", true)
				comoros.chinese_name = tr(TXT_NAME_R1)
		2:
...
# ══════════════════════════════════════════════════════════
# 自动迁移的事件定义 —— 源： 场景/事件界面/events/event_609_forever_mr_president.tres
...
const META := {
	...
	"priority": 60900,
	"notify": false,
	"display_script": "res://数据脚本/事件效果/event_609_forever_mr_president.gd",
	"trigger": [{"t": "DATE_AFTER", "key": "1978.5.12"}],
