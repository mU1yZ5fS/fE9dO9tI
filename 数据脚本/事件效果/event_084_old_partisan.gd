...
## 原作 Event84.cs：我们的老游击队员…（1978.10.4 后 且 苏 now_leader==0 勃列日涅夫 且 result[83]==0 或 data[149]==1）。
## 触发：TimeScript.cs:10691。效果（Event84.cs ResultsOfEvents）：
##  - result0（散布谣言）：data[9]-=80、苏 leaders[3](安德罗波夫).support-=2、data[149]=2
##  - result1（黑料）：苏关系>=500 → data[8]-=50、leaders[3].support-=1、data[149]=2；否则仅 data[8]-=50
##  - result2（留到将来）：无效果
